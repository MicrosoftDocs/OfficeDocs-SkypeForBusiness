...
- **Move up** Moves the selected unassigned number range up in the list so that Lync Server finds it sooner and applies the specified action before applying actions specified for other ranges in the list. 
    
    > [!NOTE]
    > Lync Server searches the unassigned number table from top to bottom and uses the first range that matches the unassigned number. For example, if you have a range that specifies a last resort action, make sure that range is at the bottom of the list. 
...
For details about Announcement features and capabilities, see [Overview of the Announcement application in Lync Server 2013](overview-of-the-announcement-application.md) in the Planning documentation. For details about working with unassigned number ranges, see [Configure unassigned phone numbers in Lync Server 2013](configure-unassigned-phone-numbers.md) in the Operations documentation. 

---
title: "Setting up support for large meetings in Lync Server 2013"
ms.author: kenwith
author: kenwith
manager: laurawi
ms.date: 11/17/2014
...
description: "In this articleTopology RequirementsHardware and Software RequirementsConfiguration Requirements"
---
...
[Topology Requirements](#sectionSection0)
  
[Hardware and Software Requirements](#sectionSection1)
  
[Configuration Requirements](#sectionSection2)
...
Managing a pool with exactly two Front End Servers requires some special considerations. For more information, see [Topologies and components for Front End Servers, instant messaging, and presence in Lync Server 2013](topologies-and-components-for-front-end-servers-instant-messaging-and-presence.md).
...
![Large Meetings pool configuration](media/Plan_LyncServer_Conf_LargeMeetingPool.jpg)
  
Additional notes about the topology include:
  
- A file share is required for storing meeting content and, if Archiving Server is deployed and enabled, for storing the archiving files. The file share can be dedicated to the pool or can be the same file share used by another pool at the site in which the pool is deployed. For details about configuring the file share, see [Configure DFS file storage for Lync Server 2013](configure-dfs-file-storage.md).
...
- Load balancing of the Front End Servers requires hardware load balancing for the HTTP traffic (such as meeting content download). DNS load balancing is recommended for SIP traffic. For details see [Load balancing requirements for Lync Server 2013](load-balancing-requirements.md). 
...
The hardware requirements for servers in a dedicated large-meeting pool are the same as for your other Lync Server 2013 servers. For details about hardware requirements, see "[Server hardware platforms for Lync Server 2013](server-hardware-platforms.md).

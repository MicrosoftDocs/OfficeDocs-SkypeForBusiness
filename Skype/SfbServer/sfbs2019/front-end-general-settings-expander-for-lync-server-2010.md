...
> [!IMPORTANT]
> If you make changes to the properties in this dialog, you must publish the topology and run the Lync Server Deployment Wizard on all affected servers. After publishing the new topology, a list of affected servers where the Lync Server Deployment Wizard must be run is provided for you as a link on the successful topology publishing summary screen. For details on publishing the updated topology, see [Publish the topology in Lync Server 2013](publish-the-topology.md). For details on the Lync Server Deployment Wizard , see [Lync Server 2013 administrative tools](lync-server-administrative-tools.md). 
  
![Front End Server general properties page](media/FrontEnd_General_Settings_Expander_2010.jpg)

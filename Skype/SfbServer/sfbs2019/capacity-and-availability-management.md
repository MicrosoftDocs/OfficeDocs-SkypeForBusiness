---
title: "Capacity and availability management in Lync Server 2013"
ms.author: jambirk
author: jambirk
manager: laurawi
ms.date: 3/9/2015
...
|:-----|:-----|
|Remote users having poor audio/video performance  <br/> |Check to see whether appropriate bandwidth is available on the WAN links and if QoS is enabled and appropriately configured. Check QoE data.  <br/> |
|Overall response of the Lync environment is slow.  <br/> |Run tests to check that the existing front-end servers can deal with the load. Introduce a new front-end server if it is needed.Check SQL database response times and fix the causes for the delays (for example, improve disk I/O).  <br/> |

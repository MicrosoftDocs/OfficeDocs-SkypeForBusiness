...
Microsoft has produced a white paper on TLS available [here](https://cloudblogs.microsoft.com/microsoftsecure/2017/06/20/tls-1-2-support-at-microsoft/), and we also recommend the background reading available in [Exchange blog](https://blogs.technet.microsoft.com/exchange/2018/01/26/exchange-server-tls-guidance-part-1-getting-ready-for-tls-1-2/).
...
Except where noted in above, once TLS 1.0 and 1.1 are disabled out-of-scope servers, clients and devices will longer function properly, or at all. This may mean you need to pause and wait for updated guidance from Microsoft. Once you are satisfied you meet all requirements and have a plan to address gaps, proceed.
...
6. Ensure all in-scope clients are updated.
...
Before you proceed with the next steps, 8make sure you have completed all prerequisites and updated Skype for Business Servers8.
...
Once TLS 1.0 and 1.1 have been disabled in your environment, check to ensure that all your main workloads are functioning as expected, such as IM & Presence, P2P calls, Enterprise Voice, etc.
...
**Option 1:** Use [SmartSetup](../../deploy/install/install-skype-for-business-server.md). Note that we are updating SmartSetup to accommodate the updated SQL binaries in a future CU, and will update this blog upon release.
...
4. Create the LYNCLOCAL SQL instance by running the command below. Wait until SQLEXPR_x64.exe finishes before proceeding to the next stepL:

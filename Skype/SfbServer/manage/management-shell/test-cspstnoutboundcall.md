...
Test-CsPstnOutboundCall can also be used in server platform mode. In that case you only need to specify the SIP address of a user, and Skype for Business Server 2015 will use certificates to authenticate that user.
  
When you run the **Test-CsPstnOutboundCall** cmdlet, the cmdlet first attempts to log the test user on to Skype for Business Server 2015. If the logon succeeds, the cmdlet will then attempt to make a phone call across the PSTN gateway. This phone call will be placed using the dial plan, voice policy, and other policies and settings assigned to the test account. When the call is answered, the cmdlet sends dual-tone multi-frequency (DTMF) codes across the network in order to verify media connectivity.
...
| _UserSipAddress_ <br/> |Optional  <br/> |System.String  <br/> |SIP address for the user account to be tested. For example:  <br/>  `-SenderSipAddress "sip:kenmyer@litwareinc.com"` <br/> The UserSipAddress parameter must reference the same user account as UserCredential  <br/> This parameter is not needed if the command is using test users configured by using the CsHealthMonitoringConfiguration cmdlets.  <br/> |

...
description: "Resiliency provides high availability and disaster recovery for the Registrar pool. By providing a backup Registrar in case of failure of the primary Registrar, the backup Registrar can take over for the failed Registrar, allowing users to sign in and communicate. Users can potentially experience reduced functionality, depending on which systems failed with the primary Registrar."
---
...
- **Failure detection interval (sec)** Type the number of seconds that should elapse before it determines that the primary Registrar is failed. The default value is 120 seconds. This field is required if you select **Enable Failover and Failback**.
    
- **Fallback detection interval (sec)** Type the number of seconds that should elapse before it determines that the primary Registrar is backed up. The default value is 240 seconds. This field is required if you select **Enable Failover and Fallback**.

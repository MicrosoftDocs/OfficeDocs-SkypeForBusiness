...
- **Resolution**: If you think that you're using a valid user account and that you have the correct password, try logging on again. If that fails, use the same credentials and try to sign-in at [https://login.microsoftonline.com/](https://login.microsoftonline.com/). If you can't sign-in there, contact Microsoft Support. 
...
To use PowerShell to manage Skype for Business Online, the EnableRemotePowerShellAccess property of your tenant PowerShell policy must be set to  `True`. If it's not, your connection fails, and you receive the following error message:

...
12.	If you are using closed federation, make sure to add any sip domains from the pure online tenant as Allowed Domains in Office 365. Note that it can take some time before the change takes effect and there is no harm in doing this early, so we suggest doing well in advance of moving to step 13.
...
- If users in either deployment use Teams, they won’t be able to federate with each (or any organization), nor will they be able interop with any Skype for Business users. While in this stage, Microsoft recommends using Teams for Channels.<br><br>

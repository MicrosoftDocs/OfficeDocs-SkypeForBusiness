...
ms.collection: 
description: "Overview of using a Cloud Call Queue with Skype for Business Server 2019."
---
...
1. Create each Cloud Call Queue's on-premises resource account  by running the `New-CsHybridApplicationEndpoint` cmdlet as needed (for one or more Call Queues), and give each one a name, sip address, and so on.

    Configuring a phone number for a call Queue is required only if you want to connect callers directly to a Queue without using an Auto Attendant. Remember the hierarchy structure will be configured online, on the server we're just creating containers to arrange later.
...
1. Assign Phone System licenses to the on-premise hybrid endpoints (Call Queue placeholders) as described in  [Assign Skype for Business and Microsoft Teams licenses](https://docs.microsoft.com/en-us/skypeforbusiness/skype-for-business-and-microsoft-teams-add-on-licensing/assign-skype-for-business-and-microsoft-teams-licenses).

2. Use the procedures in [Create a Phone System call queue](../../SfbOnline/what-is-phone-system-in-office-365/create-a-phone-system-call-queue.md) to implement the Cloud Call Queue settings.  

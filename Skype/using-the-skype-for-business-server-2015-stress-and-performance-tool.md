...
-  [Create Users and Contacts](using-the-skype-for-business-server-2015-stress-and-performance-tool.md#BKMK_CreateUsersAndContacts)
    
  
-  [Configure User Profile](using-the-skype-for-business-server-2015-stress-and-performance-tool.md#BKMK_UserProfile)
    
  
-  [Run LyncPerfTool](using-the-skype-for-business-server-2015-stress-and-performance-tool.md#BKMK_RunTool)
    
  
-  [Interpreting the Results](using-the-skype-for-business-server-2015-stress-and-performance-tool.md#BKMK_Interpret)
...
9. In the **Start Index** field, give the starting number that'll be used as a suffix to the user name prefix (for example, the prefix is "TestUser", and the first name will end in "0" in the example below.)
     ...
     ![User provisioning tool showing the user creation tab.](images/591d8280-8979-4a8c-83bc-af126e87bf29.png)
...
![User provisioning tool showing the Contents Creation tab.](images/dfb7fdf1-fb97-4e8e-8608-c4995f95dd5b.png)
...
![User Provisioning tool showing the Distribution List Creation tab.](images/4b689306-70c4-4569-9842-15c73f038eb6.png)
...
![User Provisioning tool showing the Location Info Service Config tab.](images/227662a2-e0c3-4e34-ab54-5f1459344f30.png)
...
![The User Provisioning tab showing Common Configuration tab.](images/c25df343-3550-47fb-88e0-29194338fee2.png)
...
![Load Configuration Tool showing the General Scenarios tab.](images/45792e57-4322-4c20-956f-fe480b0de1a7.png)
...
- **Location Information Services -** Select a load level of either Enabled or Disabled.
    ...
    > [!NOTE]
      >  Each of the scenarios has an Advanced button located next to it, and a set of check boxes that enable variations to the default setting.>  Choosing *Ad-hoc*  will allow the tool to generate simulation of conferences that will be created throughout the hour.>  Choosing *Large Conf*  means that a Large Conference Scenario will be simulated.>  *External*  tells the tool to also simulate external users.>  These buttons and check boxes are extra values specific to each scenario and will change the behavior of the Stress and Performance Tool and make customization possible.>  For each scenario on the General Scenarios tab (except for Location Information Services), if the value of Load Level is **Custom**, then the conversation rate will be calculated using the corresponding field in the Advanced dialog box. The field name may differ, depending on the scenario, but the field description will state:  *NOTE This number will only be used if Custom is selected from the drop-down menu*  .>  The values **High**, **Medium**, and **Low**, will alter the conversation rates per modality in line with the User Model that is a balance of all the scenarios. If there's a need to change the load level per modality due to a difference in expected usage, use a Custom conversation rate.
...
![Load Configuration tool Voice Scenarios tab.](images/042e406f-5156-4095-a4eb-6298f24bb51f.png)
...
![Load Configuration tool, Web app tab.](images/505b54ef-8140-4dec-a43e-08091f592b34.png)
...
![Load Configuration tool Mobility tab.](images/30af39c2-50ea-476a-8a56-ce2ddf08517e.png)
...
![Load Configuration tool Summary tab.](images/436fb3f2-d73e-402d-bc6e-e8a6740819d2.png)
...
![The 'Load configuration files generated successfully' message box. Just click OK.](images/c3c1d4a0-cb44-4837-8124-03354f5d9d8c.png)
...
You'll need to create users, contacts, and scenarios before running the Skype for Business Server 2015 Stress and Performance Tool (LyncPerfTool.exe). For details about using the tools to perform these actions, see  [Create Users and Contacts](using-the-skype-for-business-server-2015-stress-and-performance-tool.md#BKMK_CreateUsersAndContacts) and [Configure User Profile](using-the-skype-for-business-server-2015-stress-and-performance-tool.md#BKMK_UserProfile) previously in this article. Running these tools will also generate a file that will run with the Stress and Performance tool as part of a batch file with the required parameters included.
...
> [!NOTE]
> Every instance of the Stress and Performance tool that you start will immediately begin signing in users, usually at a rate of one user per second. > The peak user sign-in rate for the pool is about 12 per second. This means that you shouldn't start more than 12 LyncPerfTool.exe instances at the same time while users are still signing in. One thousand users will take about 20 minutes to fully sign in at one per second. 
...
|||
|:-----|:-----|
|**Performance Counter** <br/> |**Description** <br/> |

...
2. Use the [Set-CsTenantPublicProvider](/powershell/module/teams/Set-CsTenantPublicProvider) cmdlet together with the ```Provider``` parameter set to ```"WindowsLive"``` so that Teams users can communicate with Skype users.

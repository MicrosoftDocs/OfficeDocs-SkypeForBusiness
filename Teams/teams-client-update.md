...
Desktop client versions that are more than three months old upon first install and/or first run of Teams have a 28-day grace period before encountering the above-mentioned servicing information. (22-Apr-RM:  I do not find this to be the case...If I install a previously clean machine with Teams MSI 1.3.00.13565, once Teams launches I receive a "Teams need Updating" screen.  If I let it sit long enough to satisfy client idle threshold, I did find it eventually updated itself to latest version as expected, without user inverention.  However, Grace period suggests the user could use teams as expected, and for the desktop client, that wasn't the case....if we mean bby grace period that they can use web client instead, seems like something to call out specifically...) During this period, the auto-update process will update the Teams client. If not updated, users will see an in-app alert encouraging them to manually update to the latest version of Teams. The users might be prompted to contact their IT admin to do the update. This includes users using the Teams desktop client as part of the Microsoft 365 Apps for enterprise bundle.

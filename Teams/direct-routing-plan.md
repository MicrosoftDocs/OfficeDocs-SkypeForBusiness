...
|Baltimore CyberTrust Root for MTLS|Baltimore CyberTrust Root Certificate must be installed in the SBC Trusted Root Store of the Teams TLS context if Mutual TLS support is enabled for the Teams connection on the SBC. For more information, see [Baltimore CyberTrust Root Certificate for Mutual TLS](#Baltimore-CyberTrust-Root-Certificate-for-Mutual-TLS)
...
## Baltimore CyberTrust Root Certificate for Mutual TLS

If the Teams connection on the SBC is configured for Mutual TLS the TLS context used by this connection must have the Baltimore CyberTrust Root installed.  If the certificate is not installed, and MTLS is configured the connection will fail.  To obtain the Baltimore CyberTrust root, see [Encryption chains for Office 365](https://docs.microsoft.com/en-us/microsoft-365/compliance/encryption-office-365-certificate-chains?view=o365-worldwide)

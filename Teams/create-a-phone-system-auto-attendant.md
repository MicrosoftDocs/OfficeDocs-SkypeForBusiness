...
- **External phone number** transfers the caller to an external phone number that you specify. Note the following:
    ...
    - In a hybrid environment, to transfer an on-premises caller, set up an on-premises user with call forwarding set to the PSTN number. To learn more, see [Auto attendant call transfer to PSTN](../Skype/SfBServer2019/plan/exchange-unified-messaging-online-migration-support.md#auto-attendant-call-transfer-to-pstn).

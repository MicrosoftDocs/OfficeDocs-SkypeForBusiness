...
Use a CQD role with both **View Reports** and **View EUII fields** set to **Yes**.
...
:::image type="content" source="media/aa-cq-historical-report-sample-aa-v310-new-explain.png" alt-text="Screenshot showing sample cloud auto attendant analytics report, version 3.1.4":::
...
1. The Auto attendant's resource account ID instead of Auto attendant names is shown.  To show all the traffic for an Auto attendant, you must select all the resource accounts assigned to the Auto attendant.
...
:::image type="content" source="media/aa-cq-historical-report-sample-cq-v310-new-explain.png" alt-text="Screenshot showing sample cloud call queue analytics report, version 3.1.4":::
...
Each report tab retrieves data for all Auto attendants, Call queues or agents in the tenant for the selected date range regardless of the specific Resource Account or Agent selected on the report.  Filtering to show only the requested information occurs locally.  Each report tab is restricted to retrieving 90,000 rows. If there's a large number of calls being processed each day, it's possible that the report doesn't show all calls for all days within the selected date range.  There's no notification when this exclusion occurs.  Try shortening the date range to avoid this issue.

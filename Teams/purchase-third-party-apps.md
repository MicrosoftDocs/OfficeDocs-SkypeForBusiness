...
Paid apps require users to authenticate to prevent unwarranted access. Different users may have different entitlement and app functionality. For authentication, app developers can use Single Sign On (SSO) using Azure Active Directory or use third-party OAuth Identity Provider. The authentication mechanism is similar for Teams or web apps.

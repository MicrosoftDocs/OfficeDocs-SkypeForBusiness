...
You can only apply this policy to users who are in Islands mode and have the **AllowOutlookAddIn** parameter set to **True** in their Teams meeting policy. For steps on how to set this policy, see [set the meeting provider for users in Islands mode](meeting-policies-in-teams-general.md#meeting-provider-for-islands-mode).
...
- Check that the user has a Teams Meeting policy that permits the Outlook Add-in. See [Manage meeting policies in Teams](meeting-policies-in-teams-general.md#allow-the-outlook-add-in) for more details.

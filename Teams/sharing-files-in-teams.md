...
When users share a file from within Teams, they can set who can access the file just like they do across Microsoft 365. They can give access to anyone, people in your organization, people with existing access, or specific people (which can include the people in a 1:1 chat, group chat, or channel).  When a file is shared, the file preview available in the message, along with all file actions such as **Download**, **Copy link**, and **Open online**. In some cases, the sharing link may not have converted to a preview by the time a user sends the message. The file preview will be generated by the system, but in this scenario, the sharing link won't be shortened to the only the file name.

...
```
  ...
  > 5. SIP OPTIONS pings MUST NOT exceed a frequency of one transaction every 60 seconds and MUST NOT be more less frequent than one transaction every 180 seconds for each configured trunk for each endpoint.

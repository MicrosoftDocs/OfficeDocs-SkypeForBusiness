...
In Microsoft Teams, users can record their Teams meetings and group calls to capture audio, video, and screen sharing activity. There is also an option for recordings to have automatic transcription, so that users can play back meeting recordings with closed captions and review important discussion items in the transcript. The recording happens in the cloud and is saved to OneDrive for Business & SharePoint Online, so users can share it securely across their organization.
...
* Displayed in the Recordings & Transcripts tab for the meeting in Teams calendar
...
>[!Note]
> The change from using Microsoft Stream (classic) to OneDrive for Business and SharePoint for meeting recordings will automatically happen in August 2021. For detailed information, see [Use OneDrive for Business and SharePoint or Stream for meeting recordings](tmr-meeting-recording-change.md).
...
For a Teams user's meetings to be recorded, OneDrive & SharePoint must be enabled for the tenant. In addition, the following prerequisites are required for both the meeting organizer and the person who is initiating the recording:
...
- To enable channel meeting recordings to be saved so channel members can't edit or download the recordings the CSTeamsMeetingPolicy -ChannelRecordingDownload seeting must be set to Block.
...
In the Microsoft Teams admin center, turn on or turn off the **Allow cloud recording** setting in the meeting policy. To learn more, see [Meeting policy settings for audio & video](meeting-policies-audio-and-video.md#allow-cloud-recording).
...
* **Allow** (default) - Saves channel meeting recordings to a "Recordings" folder in the channel. The permissions on the recording files will be based off the Channel SharePoint permissions. This is the sames as any other file uploaded for the channel.
...
> [!NOTE]
> That transcription for recorded meetings is currently only supported for users who have the language in Teams set to English and when English is spoken in the meeting. They are stored together with the meeting recordings in OneDrive and SharePoint cloud storage.
...
In the Microsoft Teams admin center, turn on or turn off the **Allow transcription** setting in the meeting policy. To learn more, see [Meeting policy settings for audio & video](meeting-policies-audio-and-video.md#allow-transcription).
...
Meeting recordings are stored in OneDrive & SharePoint cloud storage. The location and permisions depend on the type of meeting and the role of the user in the meeting. The default permissions applied to the recording are listed below, users that have full edit rights on the video recording file can change the permissions and share it later with others as needed. 

Non-Channel meetings
* The recording is stored in a folder named **Recordings** in the OneDrive of the user who clicked record. Example:  <i>recorder's OneDrive for Business</i>/**Recordings**
...
### Temporary storage when unable to upload to OneDrive & SharePoint
If a meeting recoring is not able to be uploaded to OneDrive & SharePoint it will temporarily be available for download from Teams for 21 days before it is deleted. This is not something at this point that an admin can control or manage including the ability to delete it.

Meeting recordings may end up in this temporary storage for the following reasons:
* For non-channel meetings if the user recording doesn't have a OneDrive setup or the OneDrive has reached it's storage quota
* For a channel meeting if the SharePoint site has reached it's storage quota or the site wasn't provisioned yet
* If specific OneDrive & SharePoint policies are enabled restricting users from uploading files when not on specific IP ranges, etc.
...
The size of a 1-hour recording is 400 MB. Make sure you understand the capacity required for recorded files and have sufficient storage available in OneDrive & SharePoint.  Read [Set the default storage space for OneDrive](/onedrive/set-default-storage-space) and [Manage SharePoint site storage limits](/sharepoint/manage-site-collection-storage-limits) to understand the base storage included in the subscription and how to purchase additional storage.

## Manage meeting recordings
Meeting recordings are stored as video files in OneDrive & SharePoint and follow managment and goverance options avaiable in those platforms. Read [SharePoint governance overivew](/sharepoint/governance-overview), [OneDrive guide for enterprises](/onedrive/plan-onedrive-enterprise), or [OneDrive guide for small businesses](/onedrive/one-drive-quickstart-small-business) for more information.

For non-channel meetings, the recordings are stored in the recorder's OneDrive, thus handling ownership and retention after an employee leaves will follow the normal [OneDrive for Business and SharePoint process](/onedrive/retention-and-deletion#the-onedrive-deletion-process).
...
### eDiscovery
The meeting recordings are stored in OneDrive & SharePoint, which is Microsoft 365 and Office 365 Tier-D compliant. To support e-Discovery requests for compliance admins who are interested in meeting or call recordings, the recording completed message is available in the compliance content search functionality for Microsoft Teams. Compliance admins can look for the keyword "recording" in the subject line of the item in compliance content search preview and discover meeting and call recordings in the organization. 
...
You can apply DLP policies to meeting recording files also by the ProgID property. In the DLP rule for files in SharePoint & OneDrive set the conditions to be:

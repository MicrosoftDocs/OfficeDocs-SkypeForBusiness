...
>
> The command to preemptively change the default expiration setting in Teams is currently in deployment (Set-CsTeamsMeetingPolicy -NewMeetingRecordingExpirationDays), but you may be able to see the attribute in PowerShell. This parameter is available to be set, but will not be effective until this feature is generally available. The setting is not currently available in the Teams admin centers. These settings will be available and communicated in a message center post at least 30 days before we launch the feature.
>
>

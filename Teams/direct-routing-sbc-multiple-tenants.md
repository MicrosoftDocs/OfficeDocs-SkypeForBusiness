...
After you have registered a domain name, you need to activate it by adding at least one user with Phone System (Microsoft 365 Business Voice) license and assigning a SIP address with the FQDN portion of the SIP address matching the created base domain. License can be revoked after the domain activation (it can take up to 24 hours).

> [!NOTE]
> The Carrier tenant must keep at least one Phone System (Microsoft 365 Business Voice) license assigned to the tenant to avoid removal of the Skype for Business configuration. 

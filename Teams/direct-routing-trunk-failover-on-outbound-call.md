...
# Trunk failover on outbound call -- from Teams to the Session Border Controller (SBC)

This topic describes how to avoid trunk failovers on outbound calls.
...
If Direct Routing receives any 4xx or 6xx SIP error codes in response on an outgoing Invite, the call is considered completed by default. Outgoing means a call from a Teams client to the Public Switched Telephone Network (PSTN) with the following traffic flow :Teams Client -> Direct Routing -> SBC -> Telephony network.
...
This particular SBC might be having difficulties connecting to the callee; for example, because of a network misconfiguration. However, there is one more SBC in the route which might be able to reach the callee.

For example, in the following diagram, a user makes a call to a phone number. There are two SBCs in the route that can potentially deliver this call. Initially, SBC1.contoso.com is selected for the call, but SBC1.contoso.com isn't able to reach a PTSN network due to a network issue.
...
If you configure the parameter Set-CSOnlinePSTNGateway -Identity sbc1.contoso.com -ReinviteResponceCode "408" and there is one more SBC in the route, the second SBC will be tried-- SBC2.contoso.com in the following diagram:

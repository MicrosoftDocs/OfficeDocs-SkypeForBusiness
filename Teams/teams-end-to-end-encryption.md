...
End-to-end encrypted calls can be made between two parties when the parties are using the latest version of the Teams desktop client for Windows or Mac, they are on a Mobile device with latest update for iOS and Android or they are on a Teams Rooms on Windows device using the latest update.

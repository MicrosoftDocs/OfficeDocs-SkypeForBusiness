...
1. Select the **Upload a file** option from the left navigation, browse for the file you downloaded above and click **Upload this file**.
...
1. Once the page refreshes you will see another new section **Admin consent & additional information**.
    ...
    * Next to the **Microsoft Entra tenant** field click the **Detect** button.
...
1. Next you'll need to synchronize users between your Moodle server and Microsoft Entra ID. Depending on your environment, you may select different options during this stage. Note that the configuration you set here will run with each Moodle cron run (typically once a day) to keep everything in sync. To get started:
...
1. To validate the cron jobs (and run them manually if you wish to for the first run) click the **Scheduled tasks management page** link in the **Sync users with Microsoft Entra ID** section. This will take you to the **Scheduled Tasks** page.
    * Scroll down and find the job **Sync users with Microsoft Entra ID** job and click **Run now**.

...
> [!NOTE]
> Based on customer feedback, new Office 365 Groups generated as a result of creating a team in Microsoft Teams will no longer show in Outlook by default. For customers that want to continue with the existing behavior of showing these groups in Outlook, an Exchange Online PowerShell cmdlet will be provided which can enable the group for the Outlook experience. Groups created through Outlook and then later enabled for Teams will continue to show in both Outlook and Teams. This update will gradually roll out across Outlook and Teams in the coming months. If a new team is created via the Teams admin center or PowerShell, the team is not hidden from the GAL or Microsoft Outlook by default. Run the PowerShell command 'HiddenFromExchangeClientsEnabled' for this to hide the team from the Global Address List and Outlook clients. See [Set-UnifiedGroup](https://docs.microsoft.com/powershell/module/exchange/users-and-groups/set-unifiedgroup?view=exchange-ps) for details.

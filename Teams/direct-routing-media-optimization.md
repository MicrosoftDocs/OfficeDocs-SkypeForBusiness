...
| Teams User puts call On Hold.  Music plays on PSTN end, LMO (Local Media Optimization) is working OK. Teams User resumes the call.Call to PSTN resumes fine but LMO is not working, call continues via Central (Proxy) SBC | When a user parks a call to initiate a music on hold (MoH), it is being escalated from 1:1 to a multiparty call by Call Controller to invoke Media Controller and Media Processor (serving as AVMCU mixer) through which MoH reaches a user who has been put on hold. De-escalation to a 1:1 call after un-hold never happens as per design. Disable Local Media Optimization on the Direct Routing SBC.|

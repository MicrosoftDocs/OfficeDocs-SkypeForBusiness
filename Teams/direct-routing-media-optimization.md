...
| Voice apps (for example: Auto Attendant, Call Queue). | Voice apps can be deployed in an environment with LMO configured. However, calls involving voice apps will not be optimized for Local Media Optimization, and will instead flow through the proxy SBC, except for transfers from Auto Attendant to Local Media Optimization users. In such cases, the call will follow the Local Media Optimzation optimized path, as it is a direct 1:1 call. For Location-Based Routing scenarios, see [Voice apps (Auto Attendant or Call Queue)](location-based-routing-plan.md#inbound-calls-through-voice-apps-auto-attendant-or-call-queue). |

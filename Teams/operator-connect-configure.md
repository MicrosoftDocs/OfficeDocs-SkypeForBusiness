...
3. **Provide contact information** Your contact information, including your full name and email address, is automatically shared with your operator. You can change this information later. Additionally, you need to provide company size, and you can provide your phone number. Operators use this information to contact you with more details about Operator Connect.
...
How you set up phone numbers depends on whether they're for new users or moving existing numbers from Microsoft Calling Plans or Direct Routing.
...
The emergency address is a static location associated with a number. Once you create emergency addresses in the Teams admin center, how you assign the addresses, or change them later depends on your operator.

To assign numbers to emergency addresses, your operator implements one of three scenarios:
...
- The operator assigns emergency addresses to the phone numbers, and doesn't allow you to change them. In this scenario, you need to contact your operator to make changes to phone numbers and their assigned emergency address.
...
4. **Assign numbers.** Once your operator completes the order, they upload the numbers to your tenant. You can view the numbers and the provider in the Teams admin center by going to **Voice > Phone numbers**. Assign numbers to users from the Teams admin center or by using PowerShell. For more information, see [Assign numbers](#assign-numbers).
...
2. Once your operator completes the porting order, your operator uploads the numbers to your tenant.
...
To move from Direct Routing to Operator Connect with on-premises or online phone numbers, follow these steps:
...
The amount of time it takes for the removal to take effect depends on your configuration. To check if the on-premises number is removed and the changes are synced from on-premises to Microsoft 365, run the following Teams PowerShell Module command:

```PowerShell
Get-CsOnlineUser -Identity <user> | fl RegistrarPool, OnPremLineURI, LineURI 
```

After the changes sync to Microsoft 365 online directory, the expected output is:
...
Removing the phone number can take up to 10 minutes. In rare cases, it can take up to 24 hours. To check if the phone number was removed, run the following Teams PowerShell Module command:
...
Once your operator completes the order, they upload numbers to your tenant. You can view the numbers and the provider in the Teams admin center by going to **Voice > Phone numbers**. Assign Operator Connect numbers to users by using the Teams admin center or by using  PowerShell. For more information, see [Assign numbers](#assign-numbers).
...
- If the phone number is assigned to a user, you must unassign the number. Select **Edit**, then **Remove user**. After you save your changes, select **Release**.

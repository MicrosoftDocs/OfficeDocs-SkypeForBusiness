...
The following conditions should be satisfied for the device to be eligible for automatic updates:
    - Device model should be certified. Refer to [Devices for Teams](../devices/teams-ip-phones.md) and [Teams Rooms certified systems and peripherals](https://learn.microsoft.com/microsoftteams/rooms/certified-hardware?tabs=Android) for details.
...
4. **How do I check if a device has received an update?**
...
5. **I see that the updates are happening but they are failing. What do I do?**
...
If updates are consistently failing across your inventory, you can check a few things:
    ...
    - Network configuration is done appropriately to allow download of updates. Refer to [URLs and IP address ranges for Microsoft Teams](/microsoft-365/enterprise/urls-and-ip-address-ranges#skype-for-business-online-and-microsoft-teams) for the URLs and IP address ranges that need to be whitelisted for Teams.
...
6. **The eligible count of devices doesn't seem to match the available devices.**
...
7. **When do automatic updates happen?**

...
> [!NOTE]
> If a room is released before the end of the scheduled reservation, the organizer of the meeting will receive an email message from the room declining the reservation. This message is a result of the room removing the reservation from its schedule so it can be booked by other users and can be ignored.

...
Teams shared devices should use a [Teams Rooms resource account](/rooms/create-resource-account.md&tabs=m365-admin-center%2Cgraph-powershell-password). You can either sync these accounts to Microsoft Entra ID from Active Directory or create them directly in Microsoft Entra ID. Any password expiration policies for users will also apply to accounts used on Teams shared devices, therefore, to avoid disruptions caused by password expiration policies, set the password expiration policy for shared devices to never expire.

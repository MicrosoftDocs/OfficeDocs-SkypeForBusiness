...
- **Native Bluetooth Certified Audio Peripherals**  Most Bluetooth headsets and speakerphones that are certified for Microsoft Teams are designed to be used with a USB dongle for connectivity to PCs and Mac hosts. Many users have been asking for the option to directly pair their device with the host, because they don't have enough USB ports, or they don't want to have to remember to bring the USB dongle with them when they work in different locations.
  ...
  We're happy to announce our first Native Bluetooth certified headsets (and speakerphones to come soon). Currently native bluetooth certified devices are supported in Windows PCs, with support for Mac anticipated in 2023. These headsets are tested for audio performance as well as call control and Teams button integration, either with or without the USB dongle. The performance in native bluetooth mode is impacted by the bluetooth radio on the host PC, so we recommend installing the latest firmware updates for both headset and bluetooth radio. A small portion of users may need to continue to use the USB dongle, such as when working in high density radio enviornment, if their bluetooth is heavily used by multiple paired devices, or with certain PCs that have poor performing bluetooth radios.

...
|Ask yourself|Action |
|------------|-------|
| <ul><li>Will attendees hear a notification when a user joins or exits a meeting?</li><li> What is the required length of the PIN that a meeting organizser uses to start the meeting? | To change these settings, see [Change the settings for an Audio Conferencing bridge](change-the-settings-for-an-audio-confereincing-bridge.md).|
|||

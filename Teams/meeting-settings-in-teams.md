...
3. Under **Network**, do the following:
    ...
    - To specify port ranges, next to **Select a port range for each type of real-time media traffic**, select  **Specify port ranges**, and then enter the starting and ending ports for audio, video, and screen sharing. Selecting this option is required to implement QoS.  In order for **Specify port ranges** to work, **Insert Quality of Service (QoS) markers** also need to be enabled as they are interdependent.
        > [!IMPORTANT]

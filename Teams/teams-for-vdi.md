...
2. Install the MSI to the VDI VM by running one of the following commands:
    ...
    > [!NOTE]
    > These examples also use the **ALLUSERS=1** parameter. When you set this parameter, Teams Machine-Wide Installer appears in Programs and Features in Control Panel and in Apps & features in Windows Settings for all users of the computer. It's important to understand the difference between **ALLUSERS=1** and **ALLUSER=1**. The **ALLUSER=1** parameter is used only to specify a per-machine installation in a VDI environment.

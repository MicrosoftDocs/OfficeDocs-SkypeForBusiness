...
The information collected and shown through Class Insights does comply with GDPR and other, similar, privacy-oriented regulation. It's important for IT Admins to know that the information collected at a per-user basis is intended to be used in a class context only, to allow educators to determine class behavior (we don't show information about private chat, as an example, only class activities). While these class activities can be taken down to actions at the student level, there is no positive or negative 'value' assigned to these actions by Microsoft Teams, and there is no judgemental identification of individual students based on criteria.

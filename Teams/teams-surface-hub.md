...
Before you deploy Microsoft Teams for Surface Hub, be sure you have met the operating system and hardware requirements. For more information, see the [Microsoft Surface Hub admin guide](https://docs.microsoft.com/en-us/surface-hub/).
...
1. Under the **Device Management** section, tap **+**.
 
The **Connect to work or school** dialog box will open. 

2. Enter the policy e-mail address and password when prompted.

There's a bug in the OS that doesn't automatically refresh the UI after you've entered your device management account. You'll need to close/re-open settings in order to see the account listed. 
...
4. To verify that you have what you need, you can check the registry. You should see two keys under **HKLM\Software\Microsoft\Windows\CurrentVersion\PPI\VtcCallSettings**. 
 
The **VtcAppMeetingHandlingMode** DWORD value indicates that Teams is the default app. The following values are recognized. 

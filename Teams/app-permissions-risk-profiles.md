...
A connector posts messages to a channel when events in an external system occur.
  ...
  <tr>
    <td valign="top"><ul><li>POST_MESSAGE_CHANNEL</li></ul></td>
    <td valign="top"><ul><li>REPLYTO_CONNECTOR_MESSAGE. Certain connectors support _actionable messages_, which allow users to post targeted replies to the connector message, for example by adding a response to a GitHub issue or adding a date to a Trello card.</li></ul></td>
...
> [!Note]
> It is not currently possible to know which connectors support "actionable messages" / REPLYTO_CONNECTOR_MESSAGE.

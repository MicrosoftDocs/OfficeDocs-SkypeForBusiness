...
- **Microsoft-managed rollout**: If Teams policy was set to be Microsoft-controlled, new Teams will be rolled out according to the outlined schedule. We'll attempt to uninstall classic Teams after 14 days.
- **Admin-managed rollout**: Until the start of April, admins could choose to control the new Teams rollout as outlined in [Upgrade to the new Teams client using policies](new-teams-deploy-using-policies.md). Apart from your VDI users, this option has ended. Admins will now see the following:

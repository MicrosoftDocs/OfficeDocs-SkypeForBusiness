...
To migrate your existing Shifts data to another tenant, you'll need to export Shifts schedules for a date range, modify user names (if necessary), and then import the schedules to the destination tenant. You can export up to 100 days of Shifts data at a time. If you need to export data for a longer date range, repeat the process.
...
1. In the source tenant, for each team, export the Shifts schedule:
    ...
    1. Turn on **Export in a format that can be imported**, and then select **Export**. The Shifts schedule data is exported to an Excel file.
...
1. In the destination tenant, import the schedule data for each team.
    1. In Shifts, on the **Schedule** page, go to **More options (...)** > **Upload file**.
    1. Choose the Excel file that you exported, and then select **Open**.

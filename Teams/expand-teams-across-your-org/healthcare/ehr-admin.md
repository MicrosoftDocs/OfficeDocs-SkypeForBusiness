...
Microsoft Teams Electronic Healthcare Record (EHR) connector makes it easy for clinicians to launch a virtual patient visit or consultation with another provider in Teams directly from the EHR system. The communication and collaboration of Teams makes it easy for clinicians to cut through the clutter of traditionally fragmented systems so they can spend time providing the best possible care. Built on the Microsoft 365 cloud, Teams can help provide advanced security and empower healthcare customers to achieve their HIPAA, HITRUST compliance, and related requirements. The integration lets users:
...
Before you can set up the Teams and EHR collaboration, you need the following:
...
In this step your O365 administrator must receive a valid FHIR base URL and the admin username that will approve the configuration. O365 administrator must launch the connector and sign-in to start the configuration process.

- FHIR base URL: This is a constant address that all FHIR API endpoints for the server's default FHIR version live under. (Question. What is FHIR and do we have an example of the URL?)
...
![Image of the Epic administrator configuration screen](../../media/ehr-connector-3.png)
...
![Image of the Epic administrator configuration screen](../../media/ehr-connector-6.png)
...
![Image of the Epic administrator configuration screen](../../media/ehr-connector-7.png)
...
Throughout the integration into EHR systems, Microsoft does not store any identifiable personal data of patients or healthcare providers. Medical data is not transferred or stored by any Microsoft service. For the service to effectively work the only data that is stored by Microsoft is the healthcare provider’s Epic identifier, that is used to mapped providers to meeting links. Sign-in and authentication actions are handled by each EHR system. The provider's EHR identifier is stored in one of the three geographic regions: Asia Pacific, Americas and Europe, and Middle East Asia. 

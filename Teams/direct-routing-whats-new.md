...
> [!NOTE] 
>
> If any SBC (configured under the customer or the carrier tenant with SendSipOptions set to true) is not sending SIP OPTIONS, it will be demoted. Customers who do not want that behavior should set SendSipOptins to *false* in their SBC configuration. The same applies to carrier trunks where the SBC configuration is either under the carrier or customer tenant. In either of these cases, when SendSipOptions is set to true, the SBC sends SIP OPTIONS.

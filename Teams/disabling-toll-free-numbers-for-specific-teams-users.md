...
By default, all users in your organization are enabled for using toll-free numbers, meaning that those numbers, if available, participants can use these numbers to join their meetings. If this isn't the desired behavior for some users in your organization, you can restrict specific users from using those numbers in their meetings via a toll-free number enablement control.

When toll-free numbers are disabled for a given organizer:

- A toll-free number isn't included in their meeting invites.
- Toll-free numbers aren't listed on the "Find a local number" page that is referenced in their meeting invites.

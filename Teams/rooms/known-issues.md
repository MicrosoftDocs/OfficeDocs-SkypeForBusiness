...
|  Cisco Room devices unable to join Teams meeting   |    Joining a Teams meeting using direct guest join from a Cisco Room device fails and the call ends after a user selects **Join**. |  A change in the Teams web client that enables Cisco Room devices to join Teams meetings caused this regression. We're releasing a fix for this issue that will be deployed to all tenants by 1/15/2020. Cisco and Microsoft are working together to avoid similar issues in the future.   |  None |

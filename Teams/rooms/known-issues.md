...
|  ---        |      ---             |   ---            | --- |
| Whiteboard/Content Camera fails to enhance content when edges are not detected | After updating to 4.11.12.0, when there are no whiteboard edges in view of the content camera, the camera does not enhance/overlay all content in view of the camera.| Fix to be included in a future release to enhance all content in camera view. As a workaround, painters tape applied in a box to the whiteboard can be used to temporarily provide edges the content camera can use to focus and enhance content. | None |
| Edge browser automatic launch | The Edge browser before build 97.0.1072.62 launches automatically alongside the Microsoft Teams Room app when the device starts. | This should resolve automatically, with no user interaction required, on or before Monday, January 17, 2022. If a faster resolution is required: when Edge launches alongside the Microsoft Teams Room, visit the URL edge://settings/help, and an update should automatically download and apply. In the browser, select 'Restart' once the update has finished applying. Close Azure IoT Edge, reboot the system, and the problem should be resolved. | None |
| Split Gallery Participant Video   |  When there's no shared content in meetings with more than 9 remote participants, and the meeting is in dual Front of Room displays mode, 1 video on a Front of Room display with the self-preview might appear as audio. Also, a smaller number of audio participants than the actual number of audio participants shows up on dual Front of Room displays. | Issue will be resolved in future update. | None |

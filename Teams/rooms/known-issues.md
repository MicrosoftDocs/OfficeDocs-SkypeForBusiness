...
|  Cisco Room Devices unable to join Teams Meeting   |    Joining Teams meeting using direct guest join fails, the call is ended after user selects join. |  A change in Microsoft Teams web client interface that enables Cisco Room systems using direct guest to join Teams meeting caused this regression. The hotfix is released and will be deployed to 100% of tenants by 1/15/2020. Cisco and Microsoft also added additional validation to ensure interface reliability in future.   |  None |

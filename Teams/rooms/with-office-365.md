...
> [!NOTE]
> If the password is not set to Never Expire, then upon the account reaching the expiry period, the account will no longer sign-in on the device. The password will then need to be changed for the account and also updated locally on the MTR device.

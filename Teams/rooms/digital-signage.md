...
![DS-Meeting-enlarged-console](media/digital-signage/ds-meeting-enlarged-console.jpg)
...
Currently, digital signage is not yet available on the following clients and environments:
...
> [!WARNING]
> Make sure that there is signage content assigned to your room from the third-party service. If there is no signage content from the third-party service available for your room, signage mode will not activate even when configured to do so to avoid showing blank screens on front-of-room displays.
...
> [!NOTE]
> If you add video content in your signage playlist, the screen timeout setting may not be consistently honored due to underlying operating system behavior for video playback.
...
|Regardless of the signage source, the digital signage health signal is not available.|If digital signage has been enabled but signage mode is not activated after the maintenance window, submit an issue using **Report a problem**.|
